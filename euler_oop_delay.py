...
class LaserDESolver:
    ...
    def solve(self):
        ...
        for i in range(1, len(time)):
            if i > d:
                delayed_index_E2 = i-1-d
                delayed_index_E1 = i-1-d
            else:
                delayed_index_E2 = 0  
                delayed_index_E1 = 0  

            dE1_dt = (1 + 1j * self.D_omeg) * N1[i-1] * E1[i-1] - 0.5 * E1[i-1] - 1j * self.d_omeg * E1[i-1] + self.K * np.exp(1j * self.theta) * E2[delayed_index_E2]
            dE2_dt = (1 + 1j * self.D_omeg) * N2[i-1] * E2[i-1] - 0.5 * E2[i-1] + 1j * self.d_omeg * E2[i-1] + self.K * np.exp(1j * self.theta) * E1[delayed_index_E1]
    ...
    # Find indices of local maxima
        ...
        return unique_maxima, len(unique_maxima)
